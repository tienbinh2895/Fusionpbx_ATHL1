...
//get the dnc numbers from the database ans send them as output
	if (isset($_REQUEST["column_group"])) {
		$columns = implode(",",$_REQUEST["column_group"]);
		$sql = "select " . $columns . " from v_dnc_numbers ";
		$sql .= " where domain_uuid = '".$domain_uuid."' ";
		$prep_statement = $db->prepare(check_sql($sql));
		$prep_statement->execute();
		$dnc_numbers = $prep_statement->fetchAll(PDO::FETCH_ASSOC);
		unset ($sql, $prep_statement);
		//print_r($dnc_numbers);

		download_send_headers("data_export_" . date("Y-m-d") . ".csv");
		echo array2csv($dnc_numbers);
		die();
	}

//define the columns in the array
        $columns[] = 'dnc_number_uuid';
        $columns[] = 'domain_uuid';
        $columns[] = 'dnc_number';
        $columns[] = 'carriercode';
        $columns[] = 'enabled';
